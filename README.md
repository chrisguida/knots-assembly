...
committer email Luke uses for assembly merges, and
...
every push and on a nightly cron. The workflow currently targets
**`v29.3.knots20260210`** and completes in ~2 minutes.
...
Releases known to reproduce byte-exact with this pipeline:
...
- `curl` (used to resolve orphan short SHAs against GitHub's REST
  API — see "Why a REST API call?" below).
...
- Outbound network access to `github.com` and `api.github.com` (the
  spec references `bitcoin/bitcoin`, `bitcoinknots/bitcoin`,
  `luke-jr/bitcoin`, `bitcoin-core/gui`, plus several developer
  forks).
...
A subset of dated releases reference branches that have since been
re-rebased upstream — for example, an in-flight upstream PR Luke
included as a feature, whose source branch is force-pushed forward
each time Core advances. The release-time tip SHA of such a branch
becomes "orphaned": still preserved by GitHub as a commit object,
but no longer reachable from any current ref.

GitHub's git wire protocol *does* allow fetching orphan commits by
SHA — but only if you pass the full 40-character hash. The Knots
specs store `last=<sha>` as 11-character prefixes for readability,
which the git protocol interprets as a ref name and rejects with
`couldn't find remote ref`. The REST API
(`/repos/<owner>/<repo>/commits/<sha>`) accepts any unambiguous
prefix.

`.ci/synthesize-refs.sh` therefore makes one REST call per orphan
short SHA, to expand the prefix to its full form, and then hands
that full SHA to a normal `git fetch`. No authentication is needed;
the data being retrieved is the same data published via the git
protocol, just resolved through a different transport because of
the prefix limitation.

Adding `api.github.com` to the dependency surface is a small
expansion of what was already required — every fetch target in the
pipeline is already a `github.com` URL. If GitHub ever stops being
the canonical home for these repos, the whole pipeline needs
revisiting regardless. (A more permanent solution lives upstream:
either storing full 40-char SHAs in the spec at release
finalization, or pushing immutable archive tags for release-time
tips. Both are tracked as follow-up discussions with the spec
maintainer.)
