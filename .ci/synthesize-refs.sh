...
expand_short_sha_via_rest_api() {
  ...
  local url owner_repo full
  url=$(git -C "$BITCOIN" remote get-url "$remote" 2>/dev/null) || return
  owner_repo=$(printf '%s\n' "$url" \
    | sed -nE 's#^https?://github\.com/([^/]+/[^/]+?)(\.git)?$#\1#p')
  [ -n "$owner_repo" ] || return
  command -v curl >/dev/null || return
  full=$(curl -sf --max-time 15 \
    "https://api.github.com/repos/$owner_repo/commits/$short" \
    | sed -nE 's/.*"sha":[[:space:]]*"([a-f0-9]{40})".*/\1/p' \
    | head -1)
  [ -n "$full" ] && printf '%s\n' "$full"
}
...
while IFS='|' read -r sha subject parents; do
  ...
  if [ -n "$caret_suffix" ]; then
    ...
    # Default to using last_sha as-is for the update-ref. If we end
    # up resolving it to full via the REST API below, use that full
    # form instead (cleaner, and means git doesn't have to re-resolve
    # the prefix on every later operation).
    full_sha="$last_sha"
    if ! git -C "$BITCOIN" cat-file -e "$last_sha" 2>/dev/null; then
      ...
      for remote in luke-jr upstream origin-pull origin-pull-g origin-pull-k origin; do
        ...
        # expand_short_sha_via_rest_api() above for the rationale.
        target="$last_sha"
        if [ "${#target}" -lt 40 ]; then
          resolved=$(expand_short_sha_via_rest_api "$last_sha" "$remote")
